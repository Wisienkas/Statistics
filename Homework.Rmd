...
* [Inference for Numerical Data 2](#IfND2)  
    + [5.30 - Fuel efficiency of manual and automatic cars, Part 1](#5.30)
    + [5.40 - Work hours and education, Part 3](#5.40)
* [Inference for Categorical Data](#IfCD)
    + [6.12 - Legalization of marijuana, Part 1](#6.12)
    + [6.14 - The Civil War](#6.14)
...
```{r}
{
  ...
  se <- sqrt((sd_f^2 / size_f) + (sd_m^2 / size_m))
  
  c(pi - 1.96 * se, pi + 1.96 * se)
}
```

It would appear that women in general use way more time with their children less than age 6

### <a name="5.20"></a>5.20 - Fuel efficiency of Prius ### 

Fueleconomy.gov, the official US government source for fueleconomy information, allows users to share gas mile age information on their vehicles. The histogram below shows the distribution of gas mile age in miles per gallon (MPG) from 14 users whodrive a 2012 Toyota Prius. The sample mean is 53.3 MPG and the standard deviation is 5.2 MPG. Note that these data are user estimates and since the source data cannot be verified, the accuracy of these estimates are not guaranteed.
...
Since there is no garantee that the data is even valid or accurate at all it would not a reasonable evaluation. Also only 14 users have submitted data,
which increasingly make it hard to validate as the sample size is that low. 
...
Assuming the data is in a standard distribution and the data is accurate.

H0: The drivers on fueleconomy.gov is not wrong
H1: The drivers on fueleconomy.gov is wrong

Using a 95 % confident level
...
## <a name="IfND2"></a>Inference for Numerical Data 2 ## 
...
### <a name="5.30"></a>5.30 - Fuel efficiency of manual and automatic cars, Part 1 ### 

Each year the US Environmental Protection Agency (EPA) releases fuel economy data on cars manufactured in that year. Below are summary statistics on fuel efficiency (in miles/gallon) from random samples of cars with manual and automatic transmissions manufactured in 2012. Do these data provide strong evidence of a difference between the average fuel efficiency of cars with manual and automatic transmissions in terms of their average city mileage? Assume that conditions for inference are satisfied.

_Figures in the book on page: 256._

_Sketched data from book figure_

Table | Automatic | Manual
------|-----------|--------
Mean  |   22.92   | 27.88
SD    |    5.29   |  5.01
n     |   26      | 26

Also assuming that the 26 in the sample pool for both samples are the same car apart from the gearing.

* H0: Where is no difference in miles per galon a car drives regardless gearing
* H1: Where is a difference in miles per galon a car drives regardless gearing
...
```{r}
{
  ...
  se <- sqrt((sd_a^2 / size) + (sd_m^2 / size))
  
  t <- diff / se
  
  if(pt(q = t, df = size) < 0.050) {
...
### <a name="5.40"></a>5.40 - Work hours and education, Part 3 ### 

In Exercises 5.8 and 5.10 you worked with data
from the General Social Survey in order to compare the average number of hours worked per week
by US residents with and without a college degree. However, this analysis didn’t take advantage of
the original data which contained more accurate information on educational attainment (less than
high school, high school, junior college, Bachelor’s, and graduate school). Using ANOVA, we can
consider educational attainment levels for all 1,172 respondents at once instead of re-categorizing
them into two groups. Below are the distributions of hours worked by educational attainment and
relevant summary statistics that will be helpful in carrying out this analysis.
...
So ANOVA is used to figure out if any of the categories stick out
...
```{r}
{
  ...
  ssg <- 121 * (38.67 - 40.45)^2
  ssg <- (546 * (39.6 - 40.45)^2) + ssg
  ssg <- (97 * (41.39 - 40.45)^2) + ssg
  ssg <- (253 * (42.55 - 40.45)^2) + ssg
  ssg <- (155 * (40.85 - 40.45)^2) + ssg
  
  sse <- (121 - 1) * 15.81^2
  sse <- ((546 - 1) * 14.97^2) + sse
  sse <- ((97 - 1) * 18.1^2) + sse
  sse <- ((253 - 1) * 13.62^2) + sse
  sse <- ((155 - 1) * 15.51^2) + sse
...
Using http://www.danielsoper.com/statcalc3/calc.aspx?id=7 to calculate probability:

Probability around 12 %
    
## <a name="IfCD"></a>Inference for Categorical Data ##
...
### <a name="6.12"></a>6.12 - Legalization of marijuana, Part 1 ###

The 2010 General Social Survey asked 1,259 US residents: “Do you think the use of marijuana should be made legal, or not?” 48% of the respondents said it should be made legal

#### a. Is 48% a sample statistic or a population parameter? Explain.

It is a sample statistics. The population parameter would be the result of all US residents.
...
The data has 10 failure and successes 

it also has over 30 entries

So it would be a normal distribution

#### d.  A news piece on this survey’s findings states, “Majority of Americans think marijuana should be legalized.” Based on your confidence interval, is this news piece’s statement justified?

Since the interval covers a value over 50 % it can not be rejected that a majority of americans think marijuana should be legalized

### <a name="6.14"></a>6.14 - The Civil War ### 
...
* H0: A majority of americans think the civil war is still relevant
* H1: Less than a majority of americans think the civil war is still relevant
...
Using the 90 % confident level, it is clear that the null hypothesis can not be rejected.

It is Very very very very unlikely that the Null Hypothesis would be rejected as the p value was exceptional small. 
